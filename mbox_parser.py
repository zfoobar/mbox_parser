...
# get all names in field
def get_names_clean(field):
    # find all matches with format <user@example.com> or user@example.com
    matches = re.findall('<',str(field))
    if matches:
        field = field.split("<")[0]
        return field.strip()
    else:
...
if __name__ == '__main__':
    ...
    if len(argv) != 2:
        ...
        writer.writerow(["flagged", "date", "description", "name", "from", "to", "cc", "subject", "content", "time (minutes)"])
        ...
        for email in mailbox.mbox(mbox_file):
            ...
            sent_name = get_names_clean(email["from"])
            ...
            row = rules.apply_rules(date, sent_name, sent_from, sent_to, cc, subject, contents, owners, blacklist_domains)
